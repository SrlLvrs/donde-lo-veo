<template>
  <div
    class="grid place-content-center md:text-xl lg:text-2xl h-72 md:h-96 lg:h-[32rem] xl:h-[40rem] w-full">
...
export default {
  ...
  methods: {
    async searchMethod(prompt) {
      ...
      let url = `${base_url}?api_key=${key}&language=en&query=${prompt}&page=1&include_adult=false`;
      await axios
        .get(url)
  ...
  //https://api.themoviedb.org/3/search/multi?api_key=b839a948ada200340cd923ef9f9c2759&language=es&query=${prompt}&page=1&include_adult=false
};
</script>
